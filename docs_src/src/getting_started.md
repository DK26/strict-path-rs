...
**strict-path is designed to handle paths provided from external or unknown sources.** Whether it's user input from an HTTP request, file paths from an archive, configuration from a database, or paths generated by an LLM agent—strict-path ensures these paths are mathematically proven to stay within defined boundaries, with no escapes in any shape or form (symlinks included).

It comes in two flavors:
- **StrictPath** (via `PathBoundary`): Detects and rejects escape attempts—best for archive extraction, file uploads, config loading (90% of use cases)
- **VirtualPath** (via `VirtualRoot`): Behaves as if a subdirectory is the actual root, clamping any input to that virtual boundary—best for multi-tenant systems, malware sandboxes, security research (10% of use cases)

> **API Philosophy:** The API is minimal, restrictive, and explicit—designed to prevent and easily detect both human and LLM agent API misuse.
>
> **Security Trade-off:** This crate prioritizes security above performance. It verifies paths on disk and follows symlinks for validation, rather than working lexically. If your use case doesn't involve potential symlinks and you need maximum performance, a lexical-only solution may be a better fit.

### Built on Battle-Tested Foundations

strict-path is built on dependencies crafted especially for this project:
- **[`soft-canonicalize`](https://github.com/DK26/soft-canonicalize-rs)**: Enables path canonicalization for files that do not exist yet on the system
- **[`proc-canonicalize`](https://github.com/DK26/proc-canonicalize-rs)**: Enables expected realpath behavior for paths on host Linux machines when accessing containers (Linux container support)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let data_dir = PathBoundary::try_new_create("./data/safe_area")?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Create a virtual root that maps to "./users/user_123/files" on disk
    let vroot = VirtualRoot::try_new_create("./users/user_123/files")?;

    // User thinks they're working from "/"
    let vpath = vroot.virtual_join("/documents/my-file.txt")?;

    // But it actually maps to "./users/user_123/files/documents/my-file.txt"
