# Summary

[Introduction](./getting_started.md)

---

# Tutorial

- [Tutorial: Step-by-Step Guide](./tutorial/overview.md)
  - [Stage 1: The Basic Promise](./tutorial/stage1_basic_promise.md)
  - [Stage 2: The Mix-Up Problem](./tutorial/stage2_mixup_problem.md)
  - [Stage 3: Markers to the Rescue](./tutorial/stage3_markers.md)
  - [Stage 4: Authorization with change_marker()](./tutorial/stage4_authorization.md)
  - [Stage 5: Virtual Paths for User Sandboxing](./tutorial/stage5_virtual_paths.md)
  - [Stage 6: Feature Integration](./tutorial/stage6_features.md)

---

# Core Concepts

- [Type-System Guarantees](./type_system_guarantees.md)
- [Security Methodology](./security_methodology.md)

---

# Real-World Examples

- [Examples Overview](./examples/overview.md)
  - [Web File Upload Service](./examples/web_upload_service.md)
  - [Configuration File Manager](./examples/config_manager.md)
  - [Multi-User Document Storage](./examples/multi_user_storage.md)
  - [Archive Extraction](./examples/archive_extraction.md)
  - [CLI Tool with Safe Paths](./examples/cli_tool.md)
  - [Type-Safe Context Separation](./examples/type_safe_contexts.md)

---

# Axum Web Service Tutorial

- [Tutorial Overview](./axum_tutorial/overview.md)
  - [Chapter 1: Project Setup](./axum_tutorial/chapter1_setup.md)
  - [Chapter 2: Static Assets](./axum_tutorial/chapter2_static_assets.md)
  - [Chapter 3: Per-User Storage](./axum_tutorial/chapter3_user_storage.md)

---

# Features & Integration

- [OS Standard Directories](./os_directories.md)

---

# Best Practices

- [Guidelines & Patterns](./best_practices.md)
- [Common Mistakes to Avoid](./anti_patterns.md)

---

# API Design

- [Ergonomics](./ergonomics/overview.md)
  - [Builtin I/O Operations](./ergonomics/builtin_io.md)
  - [Generic Functions & Markers](./ergonomics/generic_functions.md)
  - [Daily Usage Patterns](./ergonomics/daily_patterns.md)
  - [Interop vs Display](./ergonomics/interop_display.md)
  - [Function Signatures](./ergonomics/signatures.md)
  - [Escape Hatches](./ergonomics/escape_hatches.md)
  - [Equality & Ordering](./ergonomics/equality_ordering.md)
  - [Naming Conventions](./ergonomics/naming.md)
  - [Canonicalized vs Lexical](./ergonomics/choosing_canonicalized_vs_lexical_solution.md)
- [Design Decisions (Frustrated Rustacean's Guide)](./design_decisions.md)

---

# Internals

- [Design & Implementation](./internals.md)
- [Type-History Pattern](./type_history_design.md)
- [Development Journey](./development_story.md)
