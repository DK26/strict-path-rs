...
// Non-recursive create (VirtualPath)
let vdir = user_docs.virtual_join("archive")?;
vdir.create_dir()?; // Fails if parent missing; use create_dir_all for recursive
```
...
`strict-path` exposes a builtin for getting metadata without following symlinks:
