# strict-path — LLM Context (Full)

**Handle paths from external or unknown sources securely.** Uses Rust's type system to mathematically prove paths stay within defined boundaries—no escapes in any shape or form, symlinks included.
...
**Trade‑off:** Security > performance. Verifies paths on disk, follows symlinks. For symlink‑free + max performance, use lexical solutions.
**Foundation:** Built on `soft-canonicalize` + `proc-canonicalize` (Linux container realpath support). Full walkthrough: **[mdBook Guide](https://dk26.github.io/strict-path-rs/)**
...
- We do not implement `AsRef<Path>` on secure path values (`StrictPath`/`VirtualPath`). When an unavoidable third-party API expects `AsRef<Path>`, pass `.interop_path()`.
	Note: `.interop_path()` returns `&OsStr`, which satisfies `AsRef<Path>` — you do not need to wrap it in `Path::new(..)` or `PathBuf::from(..)`.
	For roots/policy types, use `PathBoundary::interop_path()` and `VirtualRoot::interop_path()` similarly. No type in this crate implements `AsRef<Path>` — always use `.interop_path()` for third-party APIs.
...
- No type in this crate implements `AsRef<Path>`. When an unavoidable third-party API expects `AsRef<Path>`, pass `.interop_path()` — it returns `&OsStr` which satisfies `AsRef<Path>`.
- Interop doesn't require `.unstrict()`: prefer `.interop_path()` for those third-party adapters; call `.unstrict()` only when an owned `PathBuf` is strictly required.
- `.interop_path()` returns `&OsStr` — never wrap it in `Path::new()` to use `Path::join`/`parent`; that bypasses all security. Use `strict_*` / `virtual_*` operations instead.
 ...
 - **ANTI-PATTERN**: Never use `.interop_path().to_string_lossy()` for display purposes. Use proper display methods instead:
	 ...
	 - `VirtualRoot`: `Display` trait shows `"/"` (user-facing); for real system path use `vroot.as_unvirtual().strictpath_display()`
