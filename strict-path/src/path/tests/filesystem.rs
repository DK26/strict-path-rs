...
fn test_jailed_path_collections() {
    ...
    map.insert(restricted_path.clone(), "value");
    assert_eq!(map.get(&restricted_path), Some(&"value"));

    let mut btree: BTreeMap<StrictPath, &str> = BTreeMap::new();
    btree.insert(restricted_path.clone(), "btree");
    assert_eq!(btree.get(&restricted_path), Some(&"btree"));
}
