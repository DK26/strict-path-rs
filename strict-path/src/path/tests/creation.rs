...
fn test_jailed_path_creation() {
    ...
    let restricted_path: StrictPath = temp_dir
    ...
    assert_eq!(
        restricted_path.strictpath_to_string_lossy(),
        abs_path.to_string_lossy()
    );
}

#[test]
fn test_jailed_path_debug() {
    let test_path = PathBuf::from("path");
    let temp = tempfile::tempdir().unwrap();
    let temp_dir: PathBoundary = PathBoundary::try_new(temp.path()).unwrap();
    ...
    let debug_str = format!("{restricted_path:?}");
...
fn test_virtual_path_as_unvirtual_jailed_path() {
    ...
    fn accepts_jailed_path(jp: &StrictPath) -> String {
        jp.strictpath_to_string_lossy().to_string()
    }

    // This works - explicit borrow
    let jailed_ref: &StrictPath = virtual_path.as_unvirtual();
    ...
    assert_eq!(
        jailed_ref.strictpath_to_string_lossy(),
