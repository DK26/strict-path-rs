...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let extraction_root = if let Some(override_output) = cli.output {
        ...
        let profile = config.extraction_profiles
            .get(&cli.profile)
            .ok_or_else(|| format!("Profile '{}' not found in config", cli.profile))?;
        
        println!("Using profile '{}' with {} allowed patterns", 
                 cli.profile, profile.allowed_patterns.len());
        println!("Max files: {}, Max size: {}MB", 
