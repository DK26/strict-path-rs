...
use strict_path::{VirtualPath, VirtualRoot, StrictPath, PathBoundary, serde_ext::{WithVirtualRoot, WithBoundary}};
...
impl ConfigManager {
    ...
    fn add_defaults(&self, mut builder: config::ConfigBuilder<config::builder::DefaultState>) -> Result<config::ConfigBuilder<config::builder::DefaultState>> {
    ...
    fn add_base_config(&self, builder: config::ConfigBuilder<config::builder::DefaultState>) -> Result<config::ConfigBuilder<config::builder::DefaultState>> {
    ...
    fn add_environment_config(&self, builder: config::ConfigBuilder<config::builder::DefaultState>) -> Result<config::ConfigBuilder<config::builder::DefaultState>> {
        let config_path = self.base_dir.join("config");
        ...
        println!("ℹ️  No environment-specific configuration found for: {:?}", self.environment);
        Ok(builder)
    }
    
    fn add_environment_variables(&self, builder: config::ConfigBuilder<config::builder::DefaultState>) -> Result<config::ConfigBuilder<config::builder::DefaultState>> {
    ...
    fn add_user_config(&self, builder: config::ConfigBuilder<config::builder::DefaultState>, user_path: &Path) -> Result<config::ConfigBuilder<config::builder::DefaultState>> {
        if !user_path.exists() {
            return Err(anyhow::anyhow!("User config file not found: {}", user_path.display()));
        }
        ...
        let format = match user_path.extension().and_then(|ext| ext.to_str()) {
            ...
            _ => return Err(anyhow::anyhow!("Unsupported config file format: {}", user_path.display())),
        };
    ...
    fn validate_config(&self, raw: RawAppConfig) -> Result<ValidatedAppConfig> {
        ...
        if !raw.environment_overrides.is_empty() {
            println!("🔧 Found {} environment overrides", raw.environment_overrides.len());
        ...
        let server = self.validate_server_config(&raw.server)
            .context("Failed to validate server configuration")?;
            
        // Validate database paths  
        let database = self.validate_database_config(&raw.database)
            .context("Failed to validate database configuration")?;
            
        // Validate core paths
        let paths = self.validate_paths_config(&raw.paths)
            .context("Failed to validate paths configuration")?;
            
        // Validate security paths
        let security = self.validate_security_config(&raw.security)
            .context("Failed to validate security configuration")?;
            
        // Validate logging paths
        let logging = self.validate_logging_config(&raw.logging)
    ...
    fn validate_server_config(&self, server: &RawServerConfig) -> Result<ValidatedServerConfig> {
        ...
        let upload_dir = self.deserialize_virtual_path(&server.upload_dir, &upload_vroot, "upload_dir")?;
        
        // Validate optional static_files path
        let static_files = if let Some(static_path) = &server.static_files {
            let static_boundary = PathBoundary::try_new_create(static_path)
                .with_context(|| format!("Failed to create static files directory: {}", static_path))?;
    ...
    fn validate_database_config(&self, db: &RawDatabaseConfig) -> Result<ValidatedDatabaseConfig> {
        ...
        let backup_dir = self.deserialize_virtual_path(&db.backup_dir, &backup_vroot, "backup_dir")?;
        
        // Validate optional migration directory
        let migration_dir = if let Some(migration_path) = &db.migration_dir {
            let migration_boundary = PathBoundary::try_new_create(migration_path)
                .with_context(|| format!("Failed to create migration directory: {}", migration_path))?;
            let migration_vroot = migration_boundary.virtualize();
            Some(self.deserialize_virtual_path(migration_path, &migration_vroot, "migration_dir")?)
        } else {
            None
        };
    ...
    fn validate_paths_config(&self, paths: &RawPathsConfig) -> Result<ValidatedPathsConfig> {
        ...
        let config_dir = if let Some(config_path) = &paths.config_dir {
            Some(VirtualRoot::try_new_create(config_path)
        ...
        for (i, upload_dir) in paths.allowed_upload_dirs.iter().enumerate() {
            let upload_boundary = PathBoundary::try_new_create(upload_dir)
                .with_context(|| format!("Failed to create allowed upload directory {}: {}", i, upload_dir))?;
            let upload_vroot = upload_boundary.virtualize();
            let validated_path = self.deserialize_virtual_path(upload_dir, &upload_vroot, &format!("allowed_upload_dirs[{}]", i))?;
        ...
        for (i, data_source) in paths.trusted_data_sources.iter().enumerate() {
            let source_boundary = PathBoundary::try_new_create(data_source)
                .with_context(|| format!("Failed to create trusted data source {}: {}", i, data_source))?;
            let validated_path = self.deserialize_strict_path(data_source, &source_boundary, &format!("trusted_data_sources[{}]", i))?;
            trusted_data_sources.push(validated_path);
        }
        
        println!("  ✓ Data directory: {}", data_dir.interop_path().display());
        println!("  ✓ Cache directory: {}", cache_dir.interop_path().display());  
        println!("  ✓ Temp directory: {}", temp_dir.interop_path().display());
        println!("  ✓ Log directory: {}", log_dir.interop_path().display());
        println!("  ✓ Allowed upload directories: {}", allowed_upload_dirs.len());
    ...
    fn validate_security_config(&self, security: &RawSecurityConfig) -> Result<ValidatedSecurityConfig> {
        println!("🔒 Validating security configuration...");
        
        // Validate certificate directory as StrictPath (must exist as directory)
        let cert_boundary = PathBoundary::try_new_create(&security.cert_dir)
            .with_context(|| format!("Failed to access certificate directory: {}", security.cert_dir))?;
        let cert_dir = self.deserialize_strict_path(&security.cert_dir, &cert_boundary, "cert_dir")?;
        
        // For key file, use the cert directory as boundary and create a StrictPath within it
        let key_file = cert_boundary.strict_join(
            Path::new(&security.key_file).file_name()
                .ok_or_else(|| anyhow::anyhow!("Invalid key file path: {}", security.key_file))?
        ).with_context(|| format!("Failed to validate key file in cert directory: {}", security.key_file))?;
        ...
        for (i, restricted_path) in security.restricted_paths.iter().enumerate() {
            let restricted_boundary = PathBoundary::try_new_create(restricted_path)
                .with_context(|| format!("Failed to access restricted path {}: {}", i, restricted_path))?;
            let validated_path = self.deserialize_strict_path(restricted_path, &restricted_boundary, &format!("restricted_paths[{}]", i))?;
            restricted_paths.push(validated_path);
        }
        
        println!("  ✓ Certificate directory: {}", cert_dir.strictpath_display());
        println!("  ✓ Key file: {}", key_file.strictpath_display());
        println!("  ✓ Sandbox root: {}", sandbox_root.interop_path().display());
    ...
    fn validate_logging_config(&self, logging: &RawLoggingConfig) -> Result<ValidatedLoggingConfig> {
    ...
    fn deserialize_virtual_path<T>(&self, path_str: &str, vroot: &VirtualRoot<T>, field_name: &str) -> Result<VirtualPath<T>> 
    where 
        T: Clone + Default
    {
        let json_str = format!("\"{}\"", path_str);
        let mut de = serde_json::Deserializer::from_str(&json_str);
        WithVirtualRoot(vroot).deserialize(&mut de)
            .with_context(|| format!("Failed to validate {} path '{}' with serde", field_name, path_str))
    }
    
    // Helper function to deserialize a string path into a StrictPath using serde  
    fn deserialize_strict_path<T>(&self, path_str: &str, boundary: &PathBoundary<T>, field_name: &str) -> Result<StrictPath<T>>
    where
        T: Clone + Default 
    {
        let json_str = format!("\"{}\"", path_str);
        let mut de = serde_json::Deserializer::from_str(&json_str);
        WithBoundary(boundary).deserialize(&mut de)
            .with_context(|| format!("Failed to validate {} path '{}' with serde", field_name, path_str))
    }
}
...
fn display_config_summary(config: &ValidatedAppConfig) {
    ...
    println!("  App: {} v{} (debug: {})", config.app_name, config.version, config.debug);
    println!("  Environment: {:?}", config.environment);
    println!("  Server: {}:{} ({} workers)", config.server.host, config.server.port, config.server.workers);
    println!("  Database: {} (max: {} connections)", config.database.url, config.database.max_connections);
    
    println!("\n🔒 Secure Path Summary:");
    println!("  Data directory: {}", config.paths.data_dir.interop_path().display());
    println!("  Upload directory: {}", config.server.upload_dir.virtualpath_display());
    println!("  Backup directory: {}", config.database.backup_dir.virtualpath_display());
    println!("  Certificate directory: {}", config.security.cert_dir.strictpath_display());
    println!("  Sandbox root: {}", config.security.sandbox_root.interop_path().display());
...
fn demonstrate_secure_operations(config: &ValidatedAppConfig) -> Result<()> {
    ...
    println!("  ✓ Safe upload path: {}", upload_file.virtualpath_display());
    
    // Example 2: Safe backup file creation (from VirtualPath root)
    let backup_vroot = config.database.backup_dir.as_unvirtual().clone().virtualize();
    let backup_file = backup_vroot.virtual_join("backup_20231211.sql")?;
    println!("  ✓ Safe backup path: {}", backup_file.virtualpath_display());
    
    // Example 3: Certificate validation (StrictPath - must exist)
    println!("  ✓ Certificate directory validated: {}", config.security.cert_dir.strictpath_display());
    
    // Example 4: Sandbox containment 
    match config.security.sandbox_root.clone().virtualize().virtual_join("user_data/file.txt") {
        Ok(sandboxed_path) => {
            println!("  ✓ Sandboxed path: {}", sandboxed_path.virtualpath_display());
        }
        Err(e) => {
    ...
    println!("  ✓ Server max connections: {}", config.server.max_connections);
    if let Some(ref static_files) = config.server.static_files {
        println!("  ✓ Static files directory: {}", static_files.virtualpath_display());
    }
    
    // Use database migration directory
    if let Some(ref migration_dir) = config.database.migration_dir {
        println!("  ✓ Migration directory: {}", migration_dir.virtualpath_display());
    }
    
    // Use paths configuration
    println!("  ✓ Cache directory: {}", config.paths.cache_dir.interop_path().display());
    println!("  ✓ Temp directory: {}", config.paths.temp_dir.interop_path().display());
    println!("  ✓ Log directory: {}", config.paths.log_dir.interop_path().display());
    
    if let Some(ref config_dir) = config.paths.config_dir {
        println!("  ✓ Config directory: {}", config_dir.interop_path().display());
    }
    if let Some(ref plugin_dir) = config.paths.plugin_dir {
        println!("  ✓ Plugin directory: {}", plugin_dir.virtualpath_display());
    }
    
    println!("  ✓ Trusted data sources: {} configured", config.paths.trusted_data_sources.len());
    ...
    println!("  ✓ Security key file: {}", config.security.key_file.strictpath_display());
    println!("  ✓ Allowed origins: {}", config.security.allowed_origins.join(", "));
    println!("  ✓ Session timeout: {}s", config.security.session_timeout);
    println!("  ✓ Restricted paths: {} configured", config.security.restricted_paths.len());
    ...
    println!("  ✓ Log rotation: max {}MB, {} files", config.logging.max_size_mb, config.logging.max_files);
