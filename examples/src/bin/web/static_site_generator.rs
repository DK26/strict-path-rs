...
fn build_site(
    ...
    let source_jail: Jail<SourceContent> = Jail::try_new(source_path)?;
    let output_jail: Jail<OutputSite> = Jail::try_new_create(output_path)?;
    let theme_jail: Jail<ThemeAssets> = Jail::try_new(theme_path)?;
...
fn load_templates(
    ...
    for entry in WalkDir::new(layouts_dir.systempath_as_os_str()) {
        let entry = entry?;
        if entry.file_type().is_file() {
            let path = entry.path();
            if let Some(extension) = path.extension() {
                if extension == "html" {
                    let relative = path.strip_prefix(layouts_dir.systempath_as_os_str())?;
...
fn process_content_directory(
    ...
    for entry in WalkDir::new(content_dir.systempath_as_os_str()) {
        let entry = entry?;
        if entry.file_type().is_file() {
            let path = entry.path();
            if let Some(extension) = path.extension() {
                if extension == "md" {
                    let relative = path.strip_prefix(content_dir.systempath_as_os_str())?;
...
fn copy_theme_assets(
    ...
    for entry in WalkDir::new(assets_dir.systempath_as_os_str()) {
        ...
        if entry.file_type().is_file() {
            let path = entry.path();
            let relative = path.strip_prefix(assets_dir.systempath_as_os_str())?;
            let relative_str = relative.to_string_lossy().replace('\\', "/");
...
fn copy_static_files(
    ...
    for entry in WalkDir::new(static_dir.systempath_as_os_str()) {
        let entry = entry?;
        if entry.file_type().is_file() {
            let path = entry.path();
            let relative = path.strip_prefix(static_dir.systempath_as_os_str())?;
            let relative_str = relative.to_string_lossy().replace('\\', "/");
