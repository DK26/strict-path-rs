...
/// let existing = soft_canonicalize("/tmp")?;
/// 
/// // Also works with non-existing paths
/// let non_existing = soft_canonicalize("/tmp/some/deep/non/existing/path.txt")?;
/// # Ok(())
/// # }
/// ```
pub fn soft_canonicalize<P: AsRef<Path>>(path: P) -> io::Result<PathBuf> {
    ...
    // If the entire path exists, we're done
    if current == full_path.as_path() {
        return Ok(resolved_path);
    }

    // Collect the non-existing components in correct order
    let mut missing_components = Vec::new();
    let mut temp = full_path.as_path();
    
    while temp != current {
        if let Some(file_name) = temp.file_name() {
            missing_components.push(file_name);
        }
        temp = temp.parent().ok_or_else(|| {
            io::Error::new(
    ...
    // Reverse to get correct order (deepest first -> shallowest first)
    missing_components.reverse();

    // Process each missing component, handling '..' and '.' properly
...
mod tests {
    ...
    fn create_temp_dir() -> io::Result<PathBuf> {
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let temp_name = format!("soft_canonicalize_test_{}_{}", std::process::id(), timestamp);
    ...
    fn test_relative_path() -> io::Result<()> {
        let result = soft_canonicalize("non/existing/relative/path.txt")?;
    ...
    fn test_parent_directory_traversal() -> io::Result<()> {
        ...
        let test_path = level2.join("subdir").join("..").join("..").join("..").join("target.txt");
    ...
    fn test_mixed_existing_and_nonexisting_with_traversal() -> io::Result<()> {
        ...
        let test_path = existing_dir.join("nonexisting").join("..").join("sibling.txt");
    ...
    fn test_traversal_beyond_root() -> io::Result<()> {
        ...
        let test_path = subdir.join("..").join("..").join("..").join("etc").join("passwd");
