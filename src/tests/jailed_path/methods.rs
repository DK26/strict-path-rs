...
fn test_jailed_path_try_join_and_try_parent() {
    ...
    let actual = joined.as_ref().unwrap();
...
fn test_jailed_path_pathbuf_methods() {
    ...
    let actual_with_name = with_name.as_ref().unwrap();
    assert_eq!(
    ...
    assert!(escape_attempt.unwrap().starts_with(jail_root.as_ref()));
    ...
    let actual_with_ext = with_ext.as_ref().unwrap();
    assert_eq!(
