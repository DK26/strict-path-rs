...
fn test_jail_directory_deletion() {
    ...
    assert!(jailed_path.real_path().starts_with(&canonical_jail));
    ...
    assert!(jailed_path.real_path().to_string_lossy().contains("jail"));

    // New validations might fail (depending on implementation)
    match validator.try_path("new_file.txt") {
        Ok(new_path) => {
            assert!(new_path.real_path().to_string_lossy().contains("jail"));
        }
        Err(_) => {
    ...
    for net_path in network_paths {
        match validator.try_path(net_path) {
            Ok(jailed_path) => {
                ...
                assert!(
                    jailed_path.real_path().starts_with(&canonical_temp),
                    "Network path not properly contained: {} -> {:?}",
                    net_path,
...
fn test_special_filesystem_entries() {
    ...
    for name in special_names {
        match validator.try_path(name) {
            Ok(jailed_path) => {
                let canonical_temp = temp.path().canonicalize().unwrap();
                assert!(jailed_path.real_path().starts_with(&canonical_temp));
