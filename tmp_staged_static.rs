...
fn init_site(project_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let project_jail: Jail<SourceContent> = Jail::try_new_create(project_path)?;
    ...
    for dir in dirs {
        let dir_path = project_jail.jailed_join(dir)?;
    ...
    let config_path = project_jail.jailed_join("site.yaml")?;
    let config_content = serde_yaml::to_string(&config)?;
    config_path.write_string(&config_content)?;
    println!("??  Created: site.yaml");

    // Create sample content
    create_sample_content(&project_jail)?;
    create_default_theme(&project_jail)?;
...
fn create_sample_content(
    project_jail: &Jail<SourceContent>,
...
- Secure path handling with jailed-path
- Markdown processing
- Template engine
- Static asset management
"#;

    let index_path = project_jail.jailed_join("src/pages/index.md")?;
...
"#;
    ...
    let post_path = project_jail.jailed_join("src/posts/first-post.md")?;
...
<body>
    ...
    let base_path = project_jail.jailed_join("themes/default/layouts/base.html")?;
...
{% endblock %}"#;
    ...
    let page_path = project_jail.jailed_join("themes/default/layouts/page.html")?;
...
{% endblock %}"#;
    ...
    let post_path = project_jail.jailed_join("themes/default/layouts/post.html")?;
...
"#;
    ...
    let css_path = project_jail.jailed_join("themes/default/assets/style.css")?;
...
fn build_site(
    ...
    // Create secure jails for different content areas
    ...
    // Process pages
    let pages = process_content_directory(&source_jail, "pages")?;
    all_pages.extend(pages);

    // Process posts
    let posts = process_content_directory(&source_jail, "posts")?;
        ...
        if entry.file_type().is_file() {
            ...
            if let Some(extension) = path.extension() {
                if extension == "html" {
                    ...
                    let template_path = theme_jail.jailed_join(&template_path_str)?;
...
fn process_content_directory(
    ...
    let content_dir = source_jail.jailed_join(dir_name)?;
        ...
        if entry.file_type().is_file() {
            ...
            if let Some(extension) = path.extension() {
                if extension == "md" {
                    ...
                    let content_path = source_jail.jailed_join(&content_path_str)?;
...
fn process_markdown_file(
    file_path: &JailedPath<SourceContent>,
    relative_path: &str,
...
fn generate_page_html(
    ...
    let output_file = output_jail.jailed_join(&output_path)?;
...
fn copy_theme_assets(
    ...
    let output_assets = output_jail.jailed_join("assets")?;
    ...
    for entry in WalkDir::new(assets_dir.interop_path()) {
        ...
        if entry.file_type().is_file() {
            ...
            let source_path = theme_jail.jailed_join(format!("assets/{relative_str}"))?;
            let dest_path = output_jail.jailed_join(format!("assets/{relative_str}"))?;
...
fn copy_static_files(
    ...
    for entry in WalkDir::new(static_dir.interop_path()) {
        ...
        if entry.file_type().is_file() {
            ...
            // Validate paths through jails
            let source_path = source_jail.jailed_join(format!("static/{relative_str}"))?;
            let dest_path = output_jail.jailed_join(&relative_str)?;
