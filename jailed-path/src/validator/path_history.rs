...
impl<H> PathHistory<H> {
    ...
    /// Virtualizes this path by preparing a jail-anchored system path for validation.
    ///
    /// Semantics:
    ...
    /// - Does not resolve symlinks; that is handled by canonicalization in `Jail::jailed_join`.
    /// - Returns a path under the jail root to be canonicalized and boundary-checked.
    pub fn virtualize_to_jail<Marker>(
        self,
        jail: &crate::validator::jail::Jail<Marker>,
    ...
    /// Canonicalize relative to a jail root using anchored semantics (virtual clamp + resolution).
    /// Returns a Canonicalized state; boundary checking is still required.
    pub fn canonicalize_anchored<Marker>(
        self,
        anchor: &crate::validator::jail::Jail<Marker>,
...
impl<H> PathHistory<(H, Canonicalized)> {
    ...
    pub fn boundary_check(
        self,
        jail: &PathHistory<((Raw, Canonicalized), Exists)>,
