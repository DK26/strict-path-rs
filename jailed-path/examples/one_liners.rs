...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vp = VirtualRoot::<()>::try_new_create("quick_vroot")?
        .virtualpath_join("nested/deep/file.txt")?;
    ...
    struct Demo;
    let safe_path = Jail::<Demo>::try_new_create("demo_jail")?.systempath_join("demo.txt")?;
