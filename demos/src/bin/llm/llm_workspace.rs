...
fn apply_suggestion(suggested_file: &VirtualPath<AiWorkspace>) -> Result<()> {
    // Create parent dir if needed (virtual semantics)
    suggested_file.create_parent_dir_all()?;
    // Log, then write
    let sys = suggested_file.as_unvirtual().strictpath_display();
    println!("Writing to system path: {sys}");
    suggested_file.write("Generated by LLM tool\n")?;
    Ok(())
}
