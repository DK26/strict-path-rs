...
fn backup_volume(cli: &Cli, volume_name: &str, backup_name: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if data_root.exists() {
        for e in walkdir::WalkDir::new(data_root.interop_path()) {
            let e = e?;
            if !e.file_type().is_file() { continue; }
            let p = e.path();
            let rel = match p.strip_prefix(data_root.interop_path()) { Ok(r) => r, Err(_) => continue };
...
fn restore_volume(cli: &Cli, volume_name: &str, backup_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for e in walkdir::WalkDir::new(backup_data_dir.interop_path()) {
        let e = e?;
        if !e.file_type().is_file() { continue; }
        let p = e.path();
        let rel = match p.strip_prefix(backup_data_dir.interop_path()) { Ok(r) => r, Err(_) => continue };
        ...
        if let Ok(path) = volumes_dir.strict_join(name) {
            ...
            if let Ok(m) = serde_json::from_str::<VolumeMetadata>(&meta_file.read_to_string()?) {
                if m.last_accessed < cutoff && m.container_count == 0 {
                    ...
                    if data.exists() {
                        for e in walkdir::WalkDir::new(data.interop_path()) {
                            let e = e?;
                            if e.file_type().is_file() {
                                cleaned_size += e.metadata()?.len();
                            }
                        }
                    }
                    path.remove_dir_all()?;
