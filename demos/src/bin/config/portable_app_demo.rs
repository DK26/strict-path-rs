...
//
// Demonstrates app-path + strict-path integration for a simple note app.
// - Stores all data relative to the executable (portable)
// - Uses type-safe filesystem access with Path Boundaries and VirtualRoot
...
impl QuickNotes {
    ...
    fn load_or_create_config(
        ...
        if config_path.exists() {
            ...
            let theme = if content.contains("theme = \"light\"") {
                "light"
            } else {
                "dark"
            };
